...
function groupMetricsBySection(metrics: MetricRecord[]) {
  return metricSections
    .map((section) => ({
      ...section,
      metrics: section.metrics
        .map((metricName) => metrics.find((metric) => metric.metricName === metricName))
    ...
    <main className="content-shell metrics-shell">
      <section className="panel">
        <div className="section-heading section-heading-with-selector">
          <div>
            <span className="hero-tag">Scorecard</span>
            ...
            <div className="team-section-list">
              {deliveryTeams.map((team) => (
                <section key={team.teamKey} className="team-section">
                  ...
                  <div className="team-metric-section-list">
                    {groupMetricsBySection(team.metrics).map((metricSection) => (
                      <div key={metricSection.id} className="team-metric-section">
                        <div className="team-metric-section-heading">
                          <h4>{metricSection.title}</h4>
                          <p>{metricSection.description}</p>
                        </div>
                        <div className="team-metrics">
                          {metricSection.metrics.map((metric, index) => {
